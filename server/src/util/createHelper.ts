 ...
 * @param light 是否为浅色模式
 */
export function createRandomColor(mode?: 'light' | 'dark'): string {
    ...
    if (mode === 'dark' && r > 200 && g > 200 && b > 200) {
        return createRandomColor('dark');
    }
    if (mode === 'light' && r < 50 && g < 50 && b < 50) {
        return createRandomColor('light');
    }
